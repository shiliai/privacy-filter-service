...
hooks 通过环境变量 `PRIVACY_FILTER_URL` 读取服务地址。**未设置时不走主引擎**，自动使用 Track 2 内置的本地非模型兜底（见 [macOS 专章](#macos-远程服务--内置非模型兜底)）。
...
```bash
export PRIVACY_FILTER_URL="http://192.168.88.75:8765"
export PRIVACY_FILTER_TIMEOUT_S=15     # 远程建议 10-30s
```
